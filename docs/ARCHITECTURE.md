...
插件自身的 `router_*` 原生 subagent 路径在 v0.4 全部拒绝。`route_task` 同步等待隔离的 `codex exec` 完成并直接返回 schema 强制的 receipt，因此父 Sol 不依赖 `SubagentStop` 通知重新激活。外部 agent（例如其他插件提供的 Hegel）不会被冒充成本插件角色，但会消费当前目标的唯一委派槽，避免其后再重复派 Luna/Terra。
