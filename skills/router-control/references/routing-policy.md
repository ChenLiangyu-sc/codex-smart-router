...
`GLM_FIRST` 不会覆盖 Luna 角色。时段和额度回退只影响本来属于 worker/reviewer 的 GLM 路径。工作日 `14:00 <= time < 18:00`（`Asia/Shanghai`）在委派前直接选 Terra；额度、认证和限流错误由用户级 circuit breaker 共享给所有项目。到达服务端 `next_flush_time` 后只允许一个半开探测，成功后自动恢复 GLM。
...
v0.2 仍仅允许一层委派，子 agent 不再派生子 agent；同一时间最多一个可写角色；MCP 写租约绑定调用 ID，优先在调用完成后释放，运行时漏发完成事件时只有在工作区进程锁空闲后才能自愈；子 agent 必须返回结构化 receipt；receipt 不合格时最多修复一次。
